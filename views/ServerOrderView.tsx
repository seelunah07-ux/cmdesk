    ...
    <div className="flex flex-col min-h-screen bg-gray-50 pb-32">
      <div className="bg-white p-6 pb-4 border-b flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center space-x-2">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg transition-colors ${existingOrderForTable ? 'bg-orange-500 text-white shadow-lg' : 'bg-orange-100 text-orange-600'}`}>
            {table}
          </div>
          <select 
            value={table} 
          ...
          <div>
            ...
            <p className={`text-[9px] uppercase font-bold tracking-widest mt-1 ${existingOrderForTable ? 'text-orange-500 animate-pulse' : 'text-gray-400'}`}>
      ...
      <div className="px-6 py-4 bg-white border-b">
         <div className="relative">
            <input 
              ...
              className="w-full bg-gray-50 border-0 rounded-2xl py-3 px-5 text-sm font-bold placeholder:text-gray-300 focus:ring-2 focus:ring-orange-500"
            />
      ...
      <div className="px-6 py-4 flex space-x-3 overflow-x-auto hide-scrollbar bg-white shadow-sm border-b touch-pan-x active:cursor-grabbing">
        {categories.map(cat => (
          <button
            ...
            className={`px-6 py-2 rounded-2xl whitespace-nowrap font-bold text-[10px] uppercase tracking-widest transition-all ${
              selectedCategory === cat ? 'bg-orange-500 text-white shadow-lg scale-105' : 'bg-gray-50 text-gray-400'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="p-6 grid grid-cols-2 gap-4">
        {filteredProducts.map(product => (
          <div 
            key={product.id} 
            className="bg-white rounded-[2rem] p-4 shadow-sm flex flex-col items-center text-center border border-gray-100 transition-all hover:border-orange-200"
          >
            <div 
            ...
            >
              ...
              {cart[product.id] && (
                <div className="absolute -top-1 -right-1 bg-orange-500 text-white w-8 h-8 rounded-full flex items-center justify-center font-black text-xs border-4 border-white">
                  {cart[product.id]}
                </div>
              )}
            </div>
            ...
            {cart[product.id] ? (
              <div className="flex items-center bg-gray-50 rounded-2xl p-1 mt-auto">
                ...
                <button onClick={() => addToCart(product.id)} className="w-8 h-8 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-orange-500">+</button>
              </div>
            ) : (
        ...
        <div className="fixed bottom-24 left-0 right-0 px-6 max-w-md mx-auto z-50">
          ...
          >
            <div className="flex items-center space-x-3 text-left">
               <span className="bg-orange-500 px-2 py-0.5 rounded-lg text-[10px] font-black uppercase">
