              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-black text-gray-800 uppercase text-xs flex items-center">
                    ...
                    {order.customerName && <span className="ml-2 bg-blue-100 text-blue-600 px-2 py-0.5 rounded text-[8px] tracking-normal capitalize">{order.customerName}</span>}
                  </h3>
                  <p className="text-[10px] text-gray-400 font-bold">{order.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </div>
              <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${order.status === OrderStatus.PREPARING ? 'bg-blue-100 text-blue-600' : 'bg-cyan-100 text-cyan-600'}`}>
                {order.status}
              </span>
            </div>
            ...
            <div className="flex space-x-3">
              {order.status === OrderStatus.PENDING ? (
                ...
                <button
                  ...
                  className="flex-1 bg-blue-600 text-white font-black py-4 rounded-3xl uppercase tracking-widest text-[10px] shadow-lg shadow-blue-100 active:scale-95 transition-transform"
                >
                  Marquer comme Prêt
                </button>
              )}
